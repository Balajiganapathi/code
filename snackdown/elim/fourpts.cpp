...
int has(point p1, point p2) {
    segment seg(p1, p2);
    int ret = 0;
    fo(i, 4) if(seg.contains(point(x[i], y[i]))) ret |= (1 << i);

    return ret;
}

pair<bool, triangle> solve(vi idx) {
    line l1(x[idx[0]], y[idx[0]], x[idx[1]], y[idx[1]]);
    line l2(x[idx[0]], y[idx[0]], x[idx[2]], y[idx[2]]);
    rep(i, 2, 3) {
        line l3(x[idx[i]], y[idx[i]], x[idx[3]], y[idx[3]]);
        point p1 = l1.intersection(l2);
        point p2 = l1.intersection(l3);
        point p3 = l2.intersection(l3);
        int mask = (has(p1, p2) | has(p2, p3) | has(p1, p3));
        if(mask == 15) return mp(true, triangle(p1, p2, p3));
    }

    return mp(false, triangle(0, 0, 0, 0, 0, 0));
}
...
int main() {
    ...
    while(t--) {
        ...
        do {
            ...
            if(cur.fi) {
                ans = true;
                cout << "YES " << cur.se.a.x << " " << cur.se.a.y << " " << cur.se.b.x << " " << cur.se.b.y << " " << cur.se.c.x << " " << cur.se.c.y << endl;
            }
