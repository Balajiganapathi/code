...
public:
    ...
    bool isdiagonal() const {
        if(diag1 && i == j) return true;;
        if(diag2 && i + j == n + 2) return true;
        return false;
    }

    bool neardiagonal() const {
        return distFromDiagonal() <= 3;
    }

    int distFromDiagonal() const {
        int ret = oo;
        if(diag1) ret = min(ret, diff());
        if(diag2) ret = min(ret, diff1());
...
void moveToDiagonal(Table table) {
    ...
    fo(i, n + 1) {
        if(diag1 && table.hasBall({i, i}) && table.getBall({i,i}).isPos()) table.move(Move(Pos(i, i), Vec(1, 1), 4 * n));
        if(diag2 && table.hasBall({i, n + 2 - i}) && table.getBall({i,n + 2 - i}).isPos()) table.move(Move(Pos(i, n + 2 - i), Vec(1, -1), 4 * n));
    }
    submit(table);
}
