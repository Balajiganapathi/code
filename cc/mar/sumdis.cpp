...
void change(int diff, ll &tot, int i, int j) {
    ...
    tot += (ll)diff * after * before * ed[i][j];
}
...
public:
    ...
    void add(ll d1, ll d2) {
        d.emplace_back(d1 - d2, 0);
        d2sum += d2;
    }
    ...
    ll minSum(ll c1, ll c2) {
        int till = upper_bound(all(d), mp((ll)c2 - c1, (ll)-oo)) - d.begin();
        ll ret = d2sum + d[till].se;
        ret += (ll)till * c1;
        ret += (ll)(si(d) - 1 - till) * c2;
...
public:
    ...
    void init() {
        ...
        rep(i, s, e) {
            ...
            if(i + 3 <= e) g.addEdge(i, i + 3, ed[i][2]);
        }
    ...
    ll solve(ll c1, ll c2, ll c3) {
        ...
        X1 += D[0].minSum(c1 + c2, c1 + c3);
        X1 += D[1].minSum(c1 + c2, c2 + c3);
        X1 += D[2].minSum(c1 + c3, c2 + c3);
        /*
        return summer.minSum(c1, c2);
        int till = lower_bound(all(d), mp((ll)c2 - c1, (ll)-oo)) - d.begin();
        ll ret = d2sum + d[till].se;
        ret += (ll)till * c1;
        ret += (ll)(ns - till) * c2;
        ...
        return ret;
        */
        ll ret = X1 - X2 - X3;// - (ll)ns * (c1 + c2 + c3);
        trace(X1, X2, X3, ret);
        return ret;
    }
...
void init() {
    ...
    fo(i, nb) {
        ...
        fo(i1, 3) {
            int a = blocks[i].e - i1;
            fo(j, 3) if(a + j + 1 < n) global.addEdge(a, a + j + 1, ed[a][j]);
        }
    }
...
ll solveFrom(int f, int b) {
    int bf = bidx[f];
    ll c1 = min(blocks[bf].g.dist(f, blocks[bf].e - 2) + global.dist(blocks[bf].e - 2, blocks[b].s),
            min(blocks[bf].g.dist(f, blocks[bf].e - 1) + global.dist(blocks[bf].e - 1, blocks[b].s), 
                blocks[bf].g.dist(f, blocks[bf].e) + global.dist(blocks[bf].e, blocks[b].s)));
    ll c2 = min(blocks[bf].g.dist(f, blocks[bf].e - 2) + global.dist(blocks[bf].e - 2, blocks[b].s + 1), 
            min(blocks[bf].g.dist(f, blocks[bf].e - 1) + global.dist(blocks[bf].e - 1, blocks[b].s + 1), 
                blocks[bf].g.dist(f, blocks[bf].e) + global.dist(blocks[bf].e, blocks[b].s + 1)));
    ll c3 = min(blocks[bf].g.dist(f, blocks[bf].e - 2) + global.dist(blocks[bf].e - 2, blocks[b].s + 2), 
            min(blocks[bf].g.dist(f, blocks[bf].e - 1) + global.dist(blocks[bf].e - 1, blocks[b].s + 2), 
                blocks[bf].g.dist(f, blocks[bf].e) + global.dist(blocks[bf].e, blocks[b].s + 2)));

    ll ret = blocks[b].solve(c1, c2, c3);
    trace(f, b, c1, c2, ret);
    return ret;
}
...
#ifdef TEST
        n = 18;
        fo(j, 3) fo(i, n - j - 1) ed[i][j] = rand() % 100 + 1;
