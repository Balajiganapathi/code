...
public:
    ...
    int distFromEdge() const {
        return min(min(i, n + 2 - i), min(j, n + 2 - j));
    }
    ...
    while(!q.empty()) {
        ...
        fo(i, 8) {
            ...
            rep(e, 1, 4 * n) {
                ...
                if(states[p.i][p.j].vis == mark && states[p.i][p.j] < nxt) break;
                if(table.hasBall(p) && table.getBall(p).isNeg()) break;
        ...
        for(auto cur: tmp) {
            auto p = cur.cur;
            if(states[p.i][p.j].vis == mark && states[p.i][p.j] < cur) continue;
            states[p.i][p.j] = cur;
            q.push(cur);
        }
        /*
...
void mergePos(Table &table, int cnt) {
    int scnt = 0;
    fo(i, m) if(table.balls[i].present && table.balls[i].isPos() && !table.balls[i].isLargePos()) {
...
vi orderBalls(Table &table) {
    ...
    fo(i, m) {
        ...
        ball.tscore = ball.p.distFromDiagonal();
        //ball.tscore += ball.cost - 1;
    }
    ...
    sort(all(order), [table](int i, int j) -> bool {
            ...
            int d1 = b1.p.distFromCenter(), d2 = b2.p.distFromCenter();
...
void clearDiagonal(Table &table) {
    fo(i, m) if(table.balls[i].present && table.balls[i].isNeg() && table.balls[i].p.isdiagonal()) {
        moveNeg(table, table.balls[i].p, min(1, -table.balls[i].cost));
    }
}
...
void moveToDiagonal(Table table) {
    ...
    for(int i: order) if(table.balls[i].present && table.balls[i].isPos() && !table.balls[i].p.isdiagonal()) {
        ...
        //trace(i, si(table.moves) - pm);
    }
    trace("move", si(table.moves) - pm);
    fo(i, n + 1) {
