...
public:
    ...
    State() {}
    State(int _vis, int _dist, Move _prev) {
        vis = _vis; dist = _dist; prev = _prev;
    }
} states[mx_n][mx_n];
int mark;

vector<Pos> pockets;
bool nearPocket(Pos p) {
    int s = 5;
    for(auto po: pockets) if(abs(p.i - po.i)  <= 3 && abs(p.j - po.j) <= 3) return true;
    return false;
}

bool farFromPockets(Pos p) {
    for(auto po: pockets) if(abs(p.i - po.i)  <= 50 && abs(p.j - po.j) <= 50) return false;
    return true;
}

bool isCenter(Pos p) {
    int s = 20;
    return p.i >= n/2 - s && p.i <= n/2 + s && p.j >= n/2 - s && p.j <= n / 2 + s;
}

bool nearSide(Pos p) {
    int s = 5;
    if(p.i > s && p.i + s < n + 2) return false;
    if(p.j > s && p.j + s < n + 2) return false;
    return true;
}
...
void addMovesRev(Table &table, Pos start) {
    Pos cur = start;
    while(1) {
        State &cstate = states[cur.i][cur.j];
        assert(cstate.vis == mark);
        if(cstate.dist == 0) break;
        Move move = cstate.prev;
        move.p = cur;
        trace(move.p.i, move.p.j, move.v.vi, move.v.vj, move.e, table.at[move.p.i][move.p.j], cstate.dist);
        move.v.inv();
        table.move(move);
        cur = cstate.prev.p;
    }
...
bool pocketBall(Table &table, const Pos &start, int lim) {
    ++mark;
    queue<Pos> q;
    states[start.i][start.j] = State(mark, 0, {start, {0,0}, 0});
    q.push(start);
    bool neg = table.getBall(start).isNeg();
    ...
    while(!q.empty()) {
        ...
        fo(i, 8) {
            ...
            rep(e, 1, 4 * n) {
                ...
                if(table.hasBall(p) && table.getBall(p).isNeg() != neg) break;
 
                
 
                states[p.i][p.j] = State(mark, states[cur.i][cur.j].dist + 1, Move(cur, dirs[i], e));
                if(p.ispocket()) {
                    addMoves(table, p);
                    return true;
                }
                if(states[p.i][p.j].dist < lim) q.push(p);
            }
        }
    }
...
bool moveNeg(Table &table, const Pos &start) {
    ++mark;
    queue<Pos> q;
    states[start.i][start.j] = State(mark, 0, {start, {0,0}, 0});
    q.push(start);
    ...
    while(!q.empty()) {
        auto cur = q.front(); q.pop();
        //trace(start.i, start.j, cur.i, cur.j, finish(cur), skip(cur));
        //trace(cur.i, cur.j, isCenter(cur), ((!table.hasBall(cur) || !table.getBall(cur).isPos()) && cur != start && !nearPocket(cur)));
        if(((!table.hasBall(cur) || !table.getBall(cur).isPos())) && cur != start && !cur.neardiagonal() && !cur.ispocket()) {
            addMoves(table, cur);
            return true;
        }
        if(cur.ispocket()) continue;
    ...
    /*
    rep(i, 1, n + 1) rep(j, 1, n + 1) if(!nearPocket(Pos(i, j)) && states[i][j].vis == mark) {
        addMoves(table, Pos(i, j));
    }
    */
...
bool movePos(Table &table, const Pos &start) {
    ...
    while(!q.empty()) {
        ...
        //trace(start.i, start.j, cur.i, cur.j, finish(cur), skip(cur));
        //trace(cur.i, cur.j, isCenter(cur), ((!table.hasBall(cur) || !table.getBall(cur).isPos()) && cur != start && !nearPocket(cur)));
        if(table.hasBall(cur) && cur != start && table.getBall(cur).isLargePos()) {
            addMoves(table, cur);
            return true;
        }
        if(cur.ispocket()) continue;
...
bool moveToDiagonal(Table &table, const Pos &start, int lim) {
    ...
    while(!q.empty()) {
        ...
        vector<tuple<Pos, State, int>> tmp;
        fo(i, 8) {
            Pos p = cur;
            Vec v = dirs[i];
            int ns = 0;
            rep(e, 1, 16 * n / 10) {
                ...
                if(states[p.i][p.j].vis == mark) break;
                if(table.hasBall(p) && table.getBall(p).isNeg()) break;
                if(table.hasBall(p) && table.getBall(p).isNeg()) ++ns;
 
                
                tmp.emplace_back(p, State(mark, states[cur.i][cur.j].dist + 1, Move(cur, dirs[i], e)), ns);
        ...
        sort(all(tmp), [](const auto &p1, const auto &p2) -> bool {
                    //if(gt(p1, 0).ispocket()) return true;
                    //if(gt(p2, 0).ispocket()) return false;
                    //int d1 = gt(p1, 0).diff(), d2 = gt(p2, 0).diff();
                    int e1 = gt(p1, 1).prev.e, e2 = gt(p2, 1).prev.e;
                    int ns1 = gt(p1, 2), ns2 = gt(p2, 2);
                    //return e1 > e2;
...
void moveNegs(Table &table, int cnt) {
    int scnt = 0;
    fo(i, m) if(table.balls[i].present && table.balls[i].isLargeNeg() && !isCenter(table.balls[i].p)) {
        bool s = moveNeg(table, table.balls[i].p);
        scnt += s;
        //if(--cnt == 0) return;
    }
    trace(cnt, scnt);
...
void mergePos(Table &table, int cnt) {
    int scnt = 0;
    fo(i, m) if(table.balls[i].present && table.balls[i].isLargePos()) {
        bool s = movePos(table, table.balls[i].p);
        scnt += s;
        if(--cnt == 0) break;
    }
    //if(scnt) trace(cnt, scnt);
}
...
void shootPos(Table table) {
    ...
    do{
        ...
        for(int i: order) {
            ++idx;
            if(table.balls[i].present && table.balls[i].isPos()) {
                pocketBall(table, table.balls[i].p, min(lim, table.balls[i].cost));
                mergePos(table, (m - i) / 10);
            }
        }
        ++lim;
    } while(--tries > 0);
...
void removeSmallNegs(Table &table) {
    if(!table.hasNeg()) return;
    fo(i, m) if(table.balls[i].present && !table.balls[i].isLargeNeg() && nearPocket(table.balls[i].p)) {
        pocketBall(table, table.balls[i].p, 2);
    }
}

void clearDiagonal(Table &table) {
    fo(i, m) if(table.balls[i].present && table.balls[i].isNeg() && table.balls[i].p.isdiagonal()) {
        moveNeg(table, table.balls[i].p);
    }
}
...
void moveToDiagonal(Table &table) {
    ...
    for(int i: order) if(table.balls[i].present && table.balls[i].isPos() && !table.balls[i].p.isdiagonal()) {
        ...
        if(table.balls[i].present && table.balls[i].isLargePos()) {
            //movePos(table, table.balls[i].p);
        }
    }
    fo(i, n + 1) {
        if(table.hasBall({i, i})) table.move(Move(Pos(i, i), Vec(1, 1), 4 * n));
        //if(table.hasBall({i, n + 1 - i})) table.move(Move(Pos(i, n + 1 - i), Vec(1, -1), 4 * n));
    }
    submit(table);
}
...
void tryAllCorners(Table table) {
    //mergeNegs(table, m);
    //fromPockets(table);
    //removeSmallNegs(table);
    shootPos(table);
    //shootTo(table, {0, n + 2});
    //shootTo(table, {n + 2, 0});
    //shootTo(table, {n + 2, n + 2});
}

void moveViaDiagonal(Table table) {
    clearDiagonal(table);
    //if(table.getMinCost() >= -3) mergePos(table, 10);
    moveToDiagonal(table);
}

void pre() {
    pockets.clear();
    for(int i = 0; i <= n + 2; i += n + 2) for(int j = 0; j <= n + 2; j += n + 2) {
        pockets.push_back(Pos(i, j));
    }
}
...
#ifdef TEST
        trace(z1[zi], z2[zi], best.score, si(best.moves), best.remPos());
        tot += best.score;
#endif
    }
