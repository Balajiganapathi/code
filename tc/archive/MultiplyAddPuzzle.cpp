...
public:
    ...
    ll solve(int n) {
        ...
        for(int i = n - 1; i >= 0; --i) {
            if(bp[i] > cur) return oo;
            trace(i, cur, bp[i]);
            ll rem = cur % bp[i];
            if(rem % a != 0) return oo;
            ret += rem / a;
            cur = (cur - rem) / bp[i];
        }
        fo(i, n) trace(i, bb[i]);
        ret = (cur == s? ret: oo);
        trace(s, t, cur, a, b, n, ret);
        return ret;
    }

    void rec(int cnt, int rem, ll cur) {
        if(rem == 0) {
            return;
        }
        rec(cnt, rem - 1, cur * b);
...
namespace moj_harness {
	...
	int run_test_case(int casenum__) {
		...
		case 0: {
			...
			long long received__      = MultiplyAddPuzzle().minimalSteps(s, t, a, b);
