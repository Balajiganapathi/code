...
public:
    ...
    pi getAt(int i, ll t) {
        ...
        pi ans(rx[i], ry[i]);
        ll times = t / s;
        ans.fi += times * acc[i].back().fi;
        ans.se += times * acc[i].back().se;
    ...
    ll getDist(int i, int j, ll t) {
        pi cur = getAt(i, t);
        ll lo = t, hi = oo;
        while(lo < hi) {
            ll m = (lo + hi) / 2;
            pi nxt = getAt(j, m);
            if(getDist(cur, nxt) <= m - t) hi = m;
            else lo = m + 1;
        }
        //trace(i, j, t, lo, getAt(i, t), getAt(j, lo));
	...
	long long getMinimum( vector <int> rabbitX, vector <int> rabbitY, vector <string> _moves ) {
        ...
        while(!pq.empty()) {
            ll d;
            int mask, cur;
            tie(d, mask, cur) = *pq.begin(); pq.erase(pq.begin());
            //trace(d, mask, cur);

            fo(nxt, n) if(!is1(mask, nxt)) {
                ll nd = getDist(cur, nxt, d);
                int nmask = (mask | (1 << nxt));
                //trace(d, mask, cur, nmask, nd, nxt);
                if(nd < dist[nmask][nxt]) {
                    pq.erase(mt(dist[nmask][nxt], nmask, nxt));
                    pq.insert(mt(nd, nmask, nxt));
                    dist[nmask][nxt] = nd;
                }
            }
        }
        ...
        ret = oo;
        fo(last, n) ret = min(ret, dist[(1 << n)-1][last]);
        if(ret >= oo) ret = -1;
