...
public:
	int countStrings( vector <int> t ) {
		...
		int ret = modpow(3, si(dis), mod);

        int sub = 0;
        set<int> p1, p2, p12;
        sub = (sub + 1ll * 2 * modpow(2, si(dis), mod)) % mod;
        sub = (sub + mod - 1) % mod;
        trace(ret, sub);
        multiset<int> ls;
        ...
        fo(i, n) if(first[i]) {
            ...
            re(j, i + 1, n) {
                ...
                if(first[j]) {
                    int c1 = si(p1), c2 = si(p2), c3 = fcnt[j + 1];
                    if(ls.empty() || *ls.rend() <= j) {
                        sub = (sub + modpow(2, c1 + c2 + c3, mod)) % mod;
                    }
                    trace(i, j, c1, c2, c3);
                    p2.insert(t[j]);
                }
