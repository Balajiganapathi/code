...
/* code */
constexpr int mx_n = 46, mx_n1 = 22;

int cst[mx_n][mx_n];
llu emask[mx_n];
llu cmask[(1 << mx_n1) + 10];
class CliqueCuts {
public:
	int sum( int n, vector <int> a, vector <int> b, vector <int> c ) {
        ...
        int cnt = 0;
        for(unsigned mask = 1; mask < (1u << n1); ++mask) {
            cmask[mask] = (1llu << n) - 1;
            fo(i, n1) if(is1(mask, i)) cmask[mask] &= emask[i];
            esum[mask] = -1;
            if((cmask[mask] & mask) == mask) {
                ++cnt;
                if(cnt >= mod) cnt -= mod;
                esum[mask] = 0;
                fo(i, n1) if(is1(mask, i) fo(j, n1) if(is1(mask, j)) {
                    esum[mask] += cst[i][j];
                    if(esum[mask] >= mod) esum[mask] -= mod;
                }
            }
        }


        for(unsigned mask = 1; mask < (1u << n2); ++mask) {
            cm = (1llu << n) - 1;
            fo(i, n2) if(is1(mask, i)) cm &= (emask[i+n1] << n1);
            if((cm & mask) == mask) {
                ++cnt;
                if(cnt >= mod) cnt -= mod;
                int es = 0;
                fo(i, n2) if(is1(mask, i) fo(j, n2) if(is1(mask, j)) {
                    es += cst[i+n1][j+n1];
                    if(es >= mod) es -= mod;
                }
                ret += es;
        ...
        ret = (2ll * ret + 1ll * cnt * (mod - sum)) % mod;
