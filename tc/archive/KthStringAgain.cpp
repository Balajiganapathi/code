...
public:
    ...
    string getS(string s, ll k) {
        trace(s, k);
        int n = si(s);
        assert(k >= 1 && k <= (1ll << n));
        if(n == 1) return s;
        vector<tuple<char, int, ll>> ranks;
        fo(i, n) {
            ranks.push_back(mt(s[i], i, (1ll << (n - i - 1)) + (i == n - 1)));
        }
        sort(all(ranks));
        trace(ranks);
        ll cur = 0, prev = 0;
        ...
        trace(s, meet_0, k, prev);
        if(idx == 0) {
            return s.substr(0, 1) + getS(s.substr(1), k - prev);
        } else {
            return getS(s.substr(1), (meet_0? k - (1ll << (n-1)): k)) + s.substr(0, 1);
        }
    }
