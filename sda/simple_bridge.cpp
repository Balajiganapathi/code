...
int depth[mx_n], low[mx_n];
int bridge_dfs(int x, int up, int d) {
    vis[x] = 1;
    seen[x] = see++;
    int cur = seen[x];
    for(int e: edge[x]) {
        int y = adj(e, x);
        if(!vis[y]) cur = min(cur, bridge_dfs(y, e));
        else if(e != up) cur = min(cur, seen[y]);
    }
    if(cur == seen[x] && up != -1) {
        trace(up);
        isbridge[up] = 1;
...
void pre() {
    ini(vis, 0);
    fo(i, n) if(!vis[i]) bridge_dfs(i, -1);
    ini(vis, 0);
    ini(par, -1);
